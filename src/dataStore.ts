...
export const loadQuizDecks = (): QuizDeck[] => {
  try {
    const saved = localStorage.getItem('language-app-quiz-decks');
  ...
  try {
    const oldSaved = localStorage.getItem('english_app_data');
    if (oldSaved !== null) {
      const defaultDeck: QuizDeck = {
        id: 'legacy-deck-1',
        title: 'Varsayılan Quiz Seti',
        dataText: oldSaved,
        date: Date.now()
      };
      return [defaultDeck];
  ...
  return [{
    id: 'default-deck-1',
    title: 'Varsayılan Quiz Seti',
    dataText: DEFAULT_TEXT_DATA,
    date: Date.now()
  }];
};

export const saveQuizDecks = (decks: QuizDeck[]) => {
  try {
    localStorage.setItem('language-app-quiz-decks', JSON.stringify(decks));
  } catch (e) {
    console.error("Local storage access denied", e);
  }
};
