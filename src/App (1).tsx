...
import { TestDeck } from './testTypes';
import { loadTestDecks, saveTestDecks } from './utils/testParser';
import { TestSolverTab } from './components/TestSolverTab';

type AppMode = 'quiz' | 'manage' | 'flashcards' | 'notes' | 'tests';
...
export default function App() {
  ...
  const [isNavigatingState, setIsNavigatingState] = React.useState(false);
  ...
  const handleFullAppExport = async () => {
    try {
      const dbExport = {
        ...
        "language-app-test-decks": typeof localStorage !== 'undefined' ? (localStorage.getItem("language-app-test-decks") || "[]") : "[]",
        "language-app-test-sessions": typeof localStorage !== 'undefined' ? (localStorage.getItem("language-app-test-sessions") || "{}") : "{}"
      };
  ...
  const handleFullAppImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data["language-app-test-decks"] !== undefined) {
          localStorage.setItem("language-app-test-decks", data["language-app-test-decks"]);
          try {
            setTestDecks(JSON.parse(data["language-app-test-decks"]));
          } catch (e) {
            console.error("Failed to parse test decks from backup", e);
          }
        }

        if (data["language-app-test-sessions"] !== undefined) {
          localStorage.setItem("language-app-test-sessions", data["language-app-test-sessions"]);
        }
      ...
      <header className="bg-zinc-950 px-4 py-3 sm:px-6 sm:py-4 shadow-md border-b border-zinc-900 sticky top-0 z-[35]">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-1 sm:gap-2 bg-zinc-900 p-1 rounded-xl max-w-full overflow-x-auto shrink-0 scrollbar-none">
            ...
            <button 
              id="nav-tab-tests"
              onClick={() => changeMode('tests')}
              className={`px-2.5 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all flex items-center gap-1.5 sm:gap-2 shrink-0 ${mode === 'tests' ? 'bg-zinc-800 shadow-sm text-purple-400' : 'text-zinc-400 hover:text-zinc-200'}`}
            >
              <CheckSquare size={15} />
              <span className="hidden sm:inline">Test Çöz</span>
            </button>
            <button 
                 ...
                 {activeDeckTab === 'words' ? (
                   ...
                   sessionWords.length === 0 && viewingPastIndex === 0 ? (
                     ...
                     (() => {
                       ...
                       return (
                       <>
                         <div className={`w-full relative aspect-[4/3] sm:aspect-[1.58] perspective-1000 mt-1 h-full min-h-[280px] sm:min-h-[350px] ${isNavigatingState ? 'pointer-events-none opacity-90' : ''}`}>
                           <AnimatePresence mode="popLayout">
                             <motion.div 
                               key={currentDisplayWord.id}
                               drag={isNavigatingState ? false : "x"}
                                dragListener={!isNavigatingState}
                         ...
                         <div className={`flex items-center gap-3 sm:gap-4 mt-5 w-full justify-center ${(viewingPastIndex > 0 || isNavigatingState) ? 'opacity-50 pointer-events-none' : ''}`}>
                           <button 
