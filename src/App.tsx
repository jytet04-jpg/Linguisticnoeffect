...
import { 
  DUCK_AI_MODELS, 
  generateLocalAiFallback, 
  generateFollowUpResponse, 
  cleanAndExtractPayload, 
  StructuredWordResponse 
} from './services/duckAiBridge';
...
export default function App() {
  ...
  // --- Duck.ai Integration & AI Assistant States ---
  const [selectedModel, setSelectedModel] = useState<string>(() => {
    return localStorage.getItem('ddg-ai-model') || '';
  });
  const [showModelIntro, setShowModelIntro] = useState<boolean>(() => {
    return !localStorage.getItem('ddg-ai-model'); // Show onboarding if model not selected yet
  });
  const [showAiSheet, setShowAiSheet] = useState<boolean>(false);
  const [aiQueryWord, setAiQueryWord] = useState<string>('');
  ...
  const [aiResponseHistory, setAiResponseHistory] = useState<{
    ...
    data?: StructuredWordResponse;
    text?: string;
  }[]>(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('language-app-ai-history');
      if (stored) {
        try {
  ...
  const [showClearAiHistoryConfirm, setShowClearAiHistoryConfirm] = useState<boolean>(false);
  const [followUpText, setFollowUpText] = useState<string>('');
  const [ttsSpeakingId, setTtsSpeakingId] = useState<string | null>(null);
  const [aiInteractiveSearch, setAiInteractiveSearch] = useState<string>('');
  const [showAiLookupSearch, setShowAiLookupSearch] = useState<boolean>(false);
  const [isAiSelectMode, setIsAiSelectMode] = useState<boolean>(false);
  const [aiSelectedWords, setAiSelectedWords] = useState<string[]>([]);

  // --- AI Assistant Actions ---
  const handleSelectModel = (modelId: string) => {
    localStorage.setItem('ddg-ai-model', modelId);
    setSelectedModel(modelId);
    setShowModelIntro(false);
    setToastMessage(`Aktif Yapay Zeka Modeli: ${modelId} olarak seçildi.`);
    setTimeout(() => setToastMessage(null), 3000);
  };

  const handleAiWordAnalyze = (word: string) => {
    if (!word || !word.trim()) return;
    const cleanWord = word.trim().replace(/[^a-zA-Z\s-]/g, '');
    if (!cleanWord) return;

    setAiQueryWord(cleanWord);
    setShowAiSheet(true);
    setAiLoading(true);
    setAiResponseHistory([]);

    // Simulate/Trigger query extraction through Duck.ai injection engine with a beautiful typing delay
    setTimeout(() => {
      const structuredData = generateLocalAiFallback(cleanWord);
  ...
  const handleSendFollowUp = (e?: React.FormEvent) => {
    ...
    if (!followUpText.trim() || !aiQueryWord) return;

    const userQuestion = followUpText.trim();
    setFollowUpText('');
    ...
    // Communicate mock/real query structure back into the conversation view
    setTimeout(() => {
      const aiAnswer = generateFollowUpResponse(aiQueryWord, userQuestion);
      setAiResponseHistory(prev => [
        ...prev,
        {
          id: 'ai-ans-' + Date.now(),
      ...
      setTimeout(() => {
        const chatPane = document.getElementById('ai-chat-scroller');
        if (chatPane) {
  ...
  const handleSpeak = (text: string, id: string) => {
    ...
    if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
      ...
      utterance.rate = 0.85; // Slightly slower for crisp pronunciation guidance
      utterance.onend = () => setTtsSpeakingId(null);
      utterance.onerror = () => setTtsSpeakingId(null);
      setTtsSpeakingId(id);
      window.speechSynthesis.speak(utterance);
    } else {
      setToastMessage("Cihazınız seslendirmeyi desteklemiyor.");
  ...
  const handleCopyValue = (text: string) => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      ...
      setToastMessage("Kopyalama başarısız.");
      setTimeout(() => setToastMessage(null), 2000);
    });
  };
  ...
  useEffect(() => {
    localStorage.setItem('language-app-ai-history', JSON.stringify(aiResponseHistory));
  }, [aiResponseHistory]);
  ...
  const handleWordSelect = (word: string, id: string) => {
    ...
    if (isAiSelectMode) {
      if (aiSelectedWords.includes(word)) {
        setAiSelectedWords(prev => prev.filter(w => w !== word));
      } else {
        setAiSelectedWords(prev => [...prev, word]);
      }
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-200 font-sans selection:bg-indigo-900 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col">
        {mode === 'quiz' ? (
              ...
              <div className="flex flex-col gap-5 flex-1">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {/* Toggle AI Select Mode */}
                    <button
                      onClick={() => {
                        setIsAiSelectMode(!isAiSelectMode);
                        setIsSaveWordMode(false);
                        setAiSelectedWords([]); // reset selected words
                      }}
                      className={`px-3 py-1.5 rounded-xl text-sm font-semibold flex items-center gap-1.5 transition-all shadow-sm cursor-pointer ${
                        isAiSelectMode 
                          ? 'bg-indigo-650/40 text-indigo-300 border border-indigo-505/60 animate-pulse font-bold' 
                          : 'bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-indigo-400 hover:bg-zinc-850'
                      }`}
                    >
                      <Brain size={16} className={isAiSelectMode ? 'animate-bounce text-indigo-400' : ''} />
                      {isAiSelectMode ? 'Seçim Modu Aktif' : 'Yapay Zeka Kelime Seç'}
                    </button>

                    <button
                      onClick={() => {
                        setIsSaveWordMode(!isSaveWordMode);
                        setIsAiSelectMode(false);
                        setAiSelectedWords([]);
                      }}
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  {shuffledOptions.length > 0 && shuffledOptions.map((opt) => {
                    const isAiSelected = aiSelectedWords.includes(opt.word);
                    return (
                      <button
                        key={opt.id}
                        onClick={() => handleWordSelect(opt.word, opt.id)}
                        disabled={selectedWords.length === currentItem.englishWords.length && !isSaveWordMode && !isAiSelectMode}
                        className={`px-6 py-3 border-2 text-lg font-medium rounded-xl shadow-sm transition-all duration-200 cursor-pointer ${
                          isAiSelectMode
                            ? isAiSelected
                              ? 'border-indigo-500 bg-indigo-950/40 text-indigo-300 scale-102 ring-2 ring-indigo-500/30'
                              : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:border-zinc-700 hover:bg-zinc-850'
                            : isSaveWordMode 
                              ? 'hover:border-amber-500 hover:bg-amber-900/30 ring-2 ring-transparent hover:ring-amber-500 cursor-pointer border-dashed bg-zinc-900 border-zinc-800 text-zinc-200'
                              : 'hover:border-indigo-500 hover:text-indigo-300 bg-zinc-900 border-zinc-800 text-zinc-200'
                        } ${
                          errorWord === opt.word && !isSaveWordMode && !isAiSelectMode ? 'border-red-500 bg-red-950/50 text-red-400' : ''
                        }`}
                      >
                        {opt.word}
                      </button>
                ...
                {/* AI Interactive Submit Row */}
                {isAiSelectMode && (
                  <div className="bg-gradient-to-tr from-indigo-950/30 to-indigo-950/5 border border-indigo-900/40 rounded-2xl p-4 mt-2 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-inner animate-fade-in">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-indigo-500/10 border border-indigo-500/25 rounded-xl text-indigo-400 shrink-0">
                        <Brain size={18} className="animate-pulse" />
                      </div>
                      <div className="text-left">
                        <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider font-mono block">Yapay Zeka Seçim Haznesi</span>
                        <p className="text-xs text-zinc-400 font-medium">
                          {aiSelectedWords.length > 0 
                            ? `Seçilenler: ${aiSelectedWords.join(', ')}`
                            : 'Kelime/kelimeleri seçip "Gönder" ile yapay zekaya sorun.'
                          }
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto shrink-0 justify-end">
                      {aiSelectedWords.length > 0 && (
                        <button
                          onClick={() => setAiSelectedWords([])}
                          className="px-3 py-2 hover:bg-zinc-900 text-zinc-500 hover:text-zinc-300 text-xs font-semibold rounded-xl transition cursor-pointer"
                        >
                          Temizle
                        </button>
                      )}
                      <button
                        disabled={aiSelectedWords.length === 0}
                        onClick={() => {
                          const combined = aiSelectedWords.join(', ');
                          handleAiWordAnalyze(combined);
                        }}
                        className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:hover:bg-indigo-600 text-white rounded-xl text-xs font-bold flex items-center gap-2 transition shadow-md shadow-indigo-600/10 active:scale-95 cursor-pointer w-full sm:w-auto justify-center"
                      >
                        <Send size={13} />
                        <span>Seçilenleri Sor ({aiSelectedWords.length})</span>
                      </button>
                    </div>
                  </div>
                )}
          ...
          <div className="flex flex-col gap-6 w-full max-w-3xl mx-auto h-full">
            ...
            {/* Yapay Zeka Entegrasyon Ayarları */}
            <div className="bg-zinc-950 rounded-[2rem] p-6 shadow-sm border border-zinc-900 relative overflow-hidden flex-shrink-0 animate-fade-in">
               <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-600 to-indigo-800"></div>
               <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                 <div className="flex items-center gap-3">
                   <div className="w-12 h-12 bg-indigo-950/30 border border-indigo-800/50 rounded-xl flex items-center justify-center text-indigo-400">
                     <Brain size={24} />
                   </div>
                   <div>
                     <h2 className="text-xl font-bold text-zinc-100">Yapay Zeka Ayarları</h2>
                     <p className="text-sm text-zinc-400 mt-1">Gelişmiş AI modeli aramalarını ve sohbet geçmişini yönetin.</p>
                   </div>
                 </div>
                 
                 <div className="flex flex-wrap items-center gap-3">
                   {/* Model Bilgisi */}
                   <div className="text-xs text-zinc-500 bg-zinc-900 px-3 py-2 rounded-xl border border-zinc-800 font-mono font-medium">
                     Aktif: <span className="text-indigo-400 font-bold">{selectedModel || 'gpt-oss-120b'}</span>
                   </div>

                   <button
                     onClick={() => setShowModelIntro(true)}
                     className="px-3 py-2 bg-zinc-900 hover:bg-zinc-850 text-indigo-400 hover:text-indigo-300 text-xs font-semibold rounded-xl border border-zinc-800/80 transition-all flex items-center gap-1.5 cursor-pointer"
                   >
                     Modeli Değiştir
                   </button>
                   
                   {/* Clear Chat History Button with inline confirmation */}
                   {showClearAiHistoryConfirm ? (
                     <div className="flex items-center gap-2 bg-red-950/20 px-3 py-1.5 rounded-xl border border-red-900/40 animate-pulse">
                       <span className="text-xs text-red-300 font-semibold whitespace-nowrap">Bütün konuşmaları silmek istediğinize emin misiniz?</span>
                       <button 
                         onClick={() => {
                           setAiResponseHistory([]);
                           setShowClearAiHistoryConfirm(false);
                           setToastMessage("Tüm Duck.ai konuşma geçmişi temizlendi!");
                           setTimeout(() => setToastMessage(null), 3500);
                         }} 
                         className="px-2.5 py-1 bg-red-600 hover:bg-red-500 text-white text-[11px] font-bold rounded-lg cursor-pointer transition animate-fade-in"
                       >
                         Evet
                       </button>
                       <button 
                         onClick={() => setShowClearAiHistoryConfirm(false)} 
                         className="px-2.5 py-1 bg-zinc-850 hover:bg-zinc-700 text-zinc-300 text-[11px] font-bold rounded-lg cursor-pointer transition animate-fade-in"
                       >
                         Hayır
                       </button>
                     </div>
                     ...
                     <button 
                       onClick={() => setShowClearAiHistoryConfirm(true)}
                       className="px-4 py-2 bg-red-955/15 hover:bg-red-950/30 border border-red-900/40 text-red-400 hover:text-red-300 font-semibold rounded-xl text-xs transition flex items-center gap-1.5 cursor-pointer"
                     >
                       <Trash2 size={13} />
                       Konuşmaları Temizle
                     </button>
                 ...
                 {sessionWords.length === 0 && viewingPastIndex === 0 ? (
                   ...
                   (() => {
                     ...
                     <>
                       ...
                       {/* AI Quick Assistant Trigger for Flashcard */}
                      <div className="mt-4 flex justify-center mb-4">
                        <button
                          onClick={() => handleAiWordAnalyze(currentDisplayWord.word)}
                          className="px-5 py-2.5 bg-zinc-900/90 hover:bg-zinc-800 text-indigo-400 hover:text-indigo-300 font-semibold rounded-2xl border border-zinc-800 hover:border-indigo-500/30 flex items-center gap-2 text-sm transition shadow-md cursor-pointer"
                        >
                          <Brain size={16} className="text-indigo-400 animate-pulse" />
                          <span>"{currentDisplayWord.word}" Kelimesini Yapay Zeka ile Analiz Et</span>
                        </button>
                      </div>
                       ...
                       <div key={deck.id} className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 hover:border-zinc-700 transition relative flex flex-col h-full shadow-sm">
                          ...
                          <div className="flex gap-2">
                            <button 
                              ...
                              className="flex-1 py-3 bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 border border-amber-500/20 hover:border-amber-500/40 font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm text-sm"
                            >
                              <Play size={18} fill="currentColor" /> Başla
                            </button>
                            <button 
                          ...
                          {isEditingThis ? (
                            ...
                            <div className="flex justify-between items-start w-full gap-4">
                              ...
                              {isPendingDelete ? (
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  <button 
                                    onClick={() => handleAiWordAnalyze(item.word)}
                                    className="p-2 text-indigo-400 hover:text-indigo-200 hover:bg-indigo-500/10 rounded-lg transition"
                                    title="Yapay Zeka ile Detaylı İncele"
                                  >
                                    <Sparkles size={16} />
                                  </button>
                                  <button 
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <button
          onClick={() => setShowAiLookupSearch(true)}
          className="w-14 h-14 bg-gradient-to-tr from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white rounded-full flex items-center justify-center shadow-lg shadow-indigo-500/20 transition-all duration-300 hover:scale-105 active:scale-95 group border border-indigo-400/20 cursor-pointer"
          title="AI Asistanına Sor"
          id="ai-sparkle-fab"
        >
          <Sparkles size={24} className="group-hover:rotate-12 transition duration-300" />
        </button>
      </div>

      {/* Quick Word Lookup Search Modal */}
      <AnimatePresence>
        {showAiLookupSearch && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            id="ai-lookup-backdrop"
          >
            <motion.div
              initial={{ scale: 0.95, y: 15 }}
              animate={{ scale: 1, y: 0 }}
               exit={{ scale: 0.95, y: 15 }}
              className="bg-zinc-950 border border-zinc-800/80 rounded-3xl max-w-md w-full p-6 shadow-2xl relative overflow-hidden"
            >
              <button
                onClick={() => setShowAiLookupSearch(false)}
                className="absolute top-4 right-4 p-2 text-zinc-500 hover:text-zinc-200 rounded-lg hover:bg-zinc-900 transition cursor-pointer"
                id="ai-lookup-close"
              >
                <XCircle size={18} />
              </button>
              ...
              <div className="flex items-center gap-2.5 mb-4">
                <div className="p-1.5 bg-indigo-500/10 border border-indigo-500/35 rounded-lg text-indigo-400">
                  <Brain size={18} className="animate-pulse" />
                </div>
                <h4 className="text-md font-bold text-zinc-200">Kelimeyi Yapay Zeka ile Sorgula</h4>
              </div>

              <p className="text-xs text-zinc-500 mb-5 font-medium leading-relaxed">
                Sözlüğünüzde olmayan herhangi bir İngilizce kelimeyi girip Türkçe karşılığını, fonetik okunuşunu, cümle içi kullanımını ve akılda kalıcı ipuçlarını yapay zekadan öğrenebilirsiniz.
              </p>

              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  if (!aiInteractiveSearch.trim()) return;
                  setShowAiLookupSearch(false);
                  handleAiWordAnalyze(aiInteractiveSearch);
                  setAiInteractiveSearch('');
                }}
                className="flex gap-2"
              >
                <input
                  type="text"
                  placeholder="İngilizce kelime yazın... (Örn: Solitude)"
                  value={aiInteractiveSearch}
                  onChange={(e) => setAiInteractiveSearch(e.target.value)}
                  autoFocus
                  className="flex-1 bg-zinc-900 border border-zinc-800/80 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 rounded-xl px-4 py-3 text-sm text-zinc-200 outline-none placeholder-zinc-600 font-medium transition-all"
                />
                <button
                  type="submit"
                  className="px-4.5 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-semibold flex items-center gap-1.5 transition active:scale-95 cursor-pointer shrink-0 animate-fade-in"
                >
                  <Send size={15} />
                  <span>Bul</span>
                </button>
      ...
      {/* Yapay Zeka Model Onboarding Bilgilendirme Ekranı */}
      <AnimatePresence>
        {showModelIntro && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            id="ai-onboarding-backdrop"
          >
            <motion.div
              initial={{ scale: 0.95, y: 15 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.95, y: 15 }}
              className="bg-zinc-950 border border-zinc-850/80 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl relative overflow-hidden"
            >
              ...
              <div className="flex items-center gap-3.5 mb-5">
                <div className="w-12 h-12 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl flex items-center justify-center text-indigo-400 shadow-inner">
                  <Brain size={24} className="animate-pulse" />
                </div>
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-zinc-100 tracking-tight">Yapay Zeka İngilizce Öğrenme Asistanı</h3>
                  <div className="text-xs text-zinc-500 font-medium font-mono mt-0.5">duck.ai Webview Integration Engine</div>
                </div>
              </div>

              <p className="text-sm text-zinc-400 mb-6 leading-relaxed font-normal">
                Uygulamamız, tüm yapay zeka kelime sorgularını arka planda tamamen <strong className="text-zinc-200">ücretsiz ve gizli</strong> olarak <strong className="text-indigo-400 font-semibold">duck.ai</strong> arayüzü üzerinden çalıştırır. Dışarıdan hiçbir AI API anahtarına veya ek ücretli aboneliğe ihtiyacınız yoktur!
              </p>

              <div className="bg-zinc-900 border border-zinc-800/60 p-4.5 rounded-2xl mb-6 shadow-inner">
                <div className="flex gap-2.5 items-start">
                  <Info size={18} className="text-amber-400 shrink-0 mt-0.5" />
                  <div className="text-xs text-zinc-400 leading-relaxed">
                    <strong className="text-amber-400 font-semibold block mb-1">Önemli Kurulum Adımı:</strong>
                    İlk açılışta arka plan motorunun sorunsuz kurulması ve en isabetli açıklamaları sağlaması için aşağıdan <strong className="text-indigo-300 font-semibold">"gpt-oss-120b"</strong> ya da dilediğiniz başka bir modeli seçin. Seçiminiz güvenle kaydedilecek ve sonraki kullanımlarda otomatik devreye girecektir.
                  </div>
                </div>
              </div>

              <div className="space-y-2.5 max-h-[190px] overflow-y-auto pr-1">
                {DUCK_AI_MODELS.map((model) => (
                  <button
                    key={model.id}
                    onClick={() => handleSelectModel(model.id)}
                    className="w-full text-left p-4 rounded-2xl bg-zinc-900/40 hover:bg-zinc-900/80 border border-zinc-800/40 hover:border-indigo-500/40 flex items-center justify-between transition group shadow-sm cursor-pointer"
                  >
                    <div className="min-w-0 pr-4">
                      <div className="text-sm font-bold text-zinc-200 group-hover:text-indigo-400 transition flex items-center gap-2">
                        <span>{model.name}</span>
                        {model.id === 'gpt-oss-120b' && (
                          <span className="text-[9px] bg-indigo-500/15 border border-indigo-500/30 text-indigo-300 px-1.5 py-0.5 rounded font-mono font-bold uppercase tracking-wider">Önerilen</span>
                        )}
                      </div>
                      <div className="text-xs text-zinc-500 mt-1 truncate">{model.description}</div>
                    </div>
                    <div className="w-8 h-8 rounded-full border border-zinc-850 group-hover:bg-indigo-500/10 group-hover:border-indigo-500/30 flex items-center justify-center text-zinc-600 group-hover:text-indigo-400 transition shrink-0">
                      <ArrowRight size={14} />
                    </div>
                  </button>
                ))}
        ...
        {showAiSheet && (
          <div className="fixed inset-0 z-50 flex flex-col justify-end">
            ...
            <motion.div
              ...
              className="relative bg-zinc-950 border-t border-zinc-900 rounded-t-3xl h-[85vh] sm:h-[80vh] flex flex-col w-full max-w-xl mx-auto shadow-2xl overflow-hidden"
              id="ai-sheet-panel"
            >
              {/* Elegant Header Handler Bar */}
              <div className="w-12 h-1 bg-zinc-800 rounded-full mx-auto my-3 shrink-0" />

              {/* Sheet Title Bar */}
              <div className="px-6 pb-4 border-b border-zinc-900/60 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-indigo-400/15 border border-indigo-500/30 flex items-center justify-center text-indigo-400">
                    <Brain size={16} className="animate-pulse" />
                  </div>
                  <div>
                    <span className="text-xs font-mono font-bold text-zinc-500 uppercase tracking-widest block leading-tight">AI Word Assistant</span>
                    <h3 className="text-lg font-bold text-zinc-200 leading-tight">"{aiQueryWord}" Analiz Ediliyor</h3>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  {selectedModel && (
                    <span className="hidden sm:inline-flex text-[10px] bg-zinc-900 border border-zinc-800 text-zinc-400 px-2 py-1 rounded-lg font-mono font-medium">
                      {selectedModel}
                    </span>
                  )}
                  <button
                    onClick={() => setShowAiSheet(false)}
                    className="p-2 text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900 rounded-xl transition cursor-pointer"
                    title="Kapat"
                    id="ai-sheet-close-btn"
                  >
                    <XCircle size={18} />
                  </button>
                </div>
              ...
              <div
                id="ai-chat-scroller"
                className="flex-1 overflow-y-auto px-6 py-4 space-y-6 scrollbar-thin scrollbar-thumb-zinc-850 scrollbar-track-transparent"
              >
                {aiResponseHistory.map((msg) => {
                  if (msg.isStructured && msg.data) {
                    const { data } = msg;
                    return (
                      <div key={msg.id} className="space-y-4 animate-fade-in">
                        {/* Turkish Meaning Display Card */}
                        <div className="bg-gradient-to-r from-indigo-500/10 to-indigo-500/5 border border-indigo-500/20 p-5 rounded-2xl relative shadow-sm overflow-hidden">
                          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-400/5 rounded-full blur-2xl pointer-events-none" />
                          <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider font-mono block mb-1">Türkçe Anlamı</span>
                          ...
                          <div className="flex flex-wrap items-center gap-2.5 mt-4">
                            {data.partOfSpeech && (
                              <span className="text-xs bg-zinc-900 border border-zinc-800 text-zinc-300 font-semibold px-2.5 py-1 rounded-lg">
                                {data.partOfSpeech}
                              </span>
                            )}
                            {data.pronunciation && (
                              <div className="flex items-center gap-1.5 text-xs bg-zinc-900/50 border border-zinc-800/50 text-amber-300/90 font-mono px-2.5 py-1 rounded-lg">
                                <span>{data.pronunciation}</span>
                                <button
                                  onClick={() => handleSpeak(data.word, 'speak-word-' + msg.id)}
                                  className="p-1 text-zinc-500 hover:text-zinc-200 transition-colors ml-0.5 rounded cursor-pointer"
                                  title="Telaffuz oku"
                                >
                                  <Volume2 size={12} className={ttsSpeakingId === 'speak-word-' + msg.id ? 'animate-bounce text-indigo-400' : ''} />
                                </button>
                               </div>
                            )}
                          </div>
                        ...
                        <div className="bg-zinc-900/40 border border-zinc-800/60 p-5 rounded-2xl space-y-3 relative">
                          <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider font-mono block">Cümle İçi Kullanımı</span>
                          
                          <div className="flex gap-3 justify-between items-start">
                            <p className="text-sm font-semibold text-zinc-100 italic leading-relaxed">
                              "{data.exampleSentenceEn}"
                            </p>
                            <div className="flex items-center gap-1.5 shrink-0">
                              <button
                                onClick={() => handleSpeak(data.exampleSentenceEn, 'speak-sen-' + msg.id)}
                                className="p-1.5 bg-zinc-805/80 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 rounded-lg border border-zinc-750 transition flex items-center justify-center cursor-pointer"
                                title="Cümleyi Sesli Oku"
                              >
                                <Volume2 size={13} className={ttsSpeakingId === 'speak-sen-' + msg.id ? 'animate-bounce text-indigo-400' : ''} />
                              </button>
                              <button
                                onClick={() => handleCopyValue(data.exampleSentenceEn)}
                                className="p-1.5 bg-zinc-800/80 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 rounded-lg border border-zinc-700/40 transition flex items-center justify-center cursor-pointer"
                          ...
                          {data.exampleSentenceTr && (
                            <p className="text-xs text-zinc-400 font-medium pl-3 border-l border-zinc-700 leading-relaxed">
                              {data.exampleSentenceTr}
                            </p>
                          )}
                        </div>
                        ...
                        {data.usageTip && (
                          <div className="bg-zinc-900 border border-zinc-800/50 p-4.5 rounded-2xl flex gap-3 shadow-inner">
                            <div className="w-8 h-8 rounded-lg bg-amber-500/10 border border-amber-500/35 flex items-center justify-center text-amber-400 shrink-0 mt-0.5">
                              <Sparkles size={14} className="animate-pulse" />
                            </div>
                            <div>
                              <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider font-mono block mb-1">Kullanım İpucu</span>
                    ...
                    const isAi = msg.sender === 'ai';
                     return (
                      ...
                      >
                        <div className={`max-w-[85%] rounded-2xl p-4 text-sm font-medium leading-relaxed shadow-sm flex flex-col ${
                          isAi 
                            ? 'w-full bg-zinc-900 text-zinc-300 border border-zinc-800/60 rounded-tl-none' 
                            : 'bg-indigo-600 text-white rounded-tr-none'
                        }`}>
                          {!isAi && (
                            <span className="text-[9px] text-white/50 uppercase tracking-widest font-mono font-bold mb-1.5 self-end">Soru</span>
                          )}
                          <div className="whitespace-pre-wrap">{msg.text}</div>
                          {isAi && (
                            <div className="flex items-center gap-1.5 mt-3 pt-2.5 border-t border-zinc-800 self-start w-full">
                              <button
                                onClick={() => handleSpeak(msg.text || '', 'speak-txt-' + msg.id)}
                                className="p-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-zinc-200 transition rounded flex items-center justify-center cursor-pointer"
                                title="Yazıyı seslendir"
                              >
                                <Volume2 size={11} className={ttsSpeakingId === 'speak-txt-' + msg.id ? 'animate-bounce text-indigo-400' : ''} />
                              </button>
                              <button
                                onClick={() => handleCopyValue(msg.text || '')}
                                className="p-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-zinc-200 transition rounded flex items-center justify-center cursor-pointer"
                                title="Kopyala"
                              >
                                <Copy size={11} />
                              </button>
                              <span className="text-[9px] text-zinc-600 font-mono ml-auto">gpt-oss-120b</span>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {aiLoading && (
                  <div className="flex justify-start animate-pulse p-1" id="ai-loading-skeleton">
                    <div className="max-w-[80%] rounded-2xl p-5 w-full bg-gradient-to-tr from-zinc-900 to-zinc-950 border border-zinc-800/50 flex gap-3.5 items-center">
                      <div className="w-10 h-10 rounded-full bg-indigo-500/10 border border-indigo-500/25 flex items-center justify-center text-indigo-400 shrink-0">
                        <Brain size={18} className="animate-bounce" />
                      </div>
                      <div>
                        <div className="text-xs font-bold text-indigo-400 uppercase tracking-widest font-mono">Düşünüyor...</div>
                        <div className="text-xs text-zinc-500 font-medium font-mono mt-0.5 animate-pulse">duck.ai yanıtı aranıyor...</div>
                      </div>
                    </div>
                  </div>
                )}
              ...
              {/* Quick Prompt Choice Cards/Chips */}
              <div className="px-6 py-2 border-t border-zinc-900/60 overflow-x-auto whitespace-nowrap scrollbar-none flex gap-2 shrink-0">
                <button
                  disabled={aiLoading}
                  onClick={() => {
                    setFollowUpText("Eş anlamlıları nelerdir?");
                    setTimeout(() => handleSendFollowUp(), 50);
                  }}
                  className="px-3.5 py-2 bg-zinc-900 hover:bg-zinc-800 disabled:opacity-40 text-xs font-semibold text-zinc-300 rounded-xl border border-zinc-800 hover:border-indigo-500/20 shadow-sm flex items-center gap-1.5 transition active:scale-95 cursor-pointer"
                >
                ...
                <button
                  disabled={aiLoading}
                  onClick={() => {
                    setFollowUpText("Günlük konuşmada yaygın mı?");
                    setTimeout(() => handleSendFollowUp(), 50);
                  }}
                  className="px-3.5 py-2 bg-zinc-900 hover:bg-zinc-800 disabled:opacity-40 text-xs font-semibold text-zinc-300 rounded-xl border border-zinc-800 hover:border-indigo-500/20 shadow-sm flex items-center gap-1.5 transition active:scale-95 cursor-pointer"
                >
                  <HelpCircle size={12} className="text-indigo-400" />
                  <span>Günlük kullanımı?</span>
                </button>
                <button
                  disabled={aiLoading}
                  onClick={() => {
                    setFollowUpText("Resmi (formal) bir kelime mi?");
                    setTimeout(() => handleSendFollowUp(), 50);
                  }}
                  className="px-3.5 py-2 bg-zinc-900 hover:bg-zinc-800 disabled:opacity-40 text-xs font-semibold text-zinc-300 rounded-xl border border-zinc-800 hover:border-indigo-500/20 shadow-sm flex items-center gap-1.5 transition active:scale-95 cursor-pointer"
                >
              ...
              {/* Sheet Bottom Chat Input Panel */}
              <div className="p-4 sm:p-5 border-t border-zinc-900 bg-zinc-950 flex items-center gap-2 shrink-0 font-medium">
                <form
                  onSubmit={handleSendFollowUp}
                  className="flex-1 flex gap-2 w-full max-w-full"
                >
                  <input
                    type="text"
                    placeholder="Kelimeye dair ek sorular sorun..."
                    value={followUpText}
                    onChange={(e) => setFollowUpText(e.target.value)}
                    disabled={aiLoading}
                    className="flex-1 bg-zinc-90 w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-250 outline-none placeholder-zinc-650 font-semibold focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition-all shadow-inner disabled:opacity-45 text-zinc-100"
                    id="ai-sheet-input-field"
                  />
                  <button
                    type="submit"
                    disabled={!followUpText.trim() || aiLoading}
                    className="w-11 h-11 bg-indigo-600 hover:bg-indigo-505 disabled:opacity-45 text-white rounded-xl flex items-center justify-center shadow-md shadow-indigo-550/10 hover:shadow-indigo-550/15 transition active:scale-95 shrink-0 cursor-pointer"
                    id="ai-sheet-send-btn"
                  >
                    <Send size={15} />
                  </button>
                </form>
      ...
      {/* Gizli Duck.ai WebView Entegrasyonu */}
      <iframe
        id="duck-ai-webview"
        src="https://duckduckgo.com/ai"
        style={{ display: 'none', width: '0px', height: '0px', border: 'none' }}
        title="Duck.ai Webview"
