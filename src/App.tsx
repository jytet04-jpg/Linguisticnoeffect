...
export default function App() {
  ...
  const parsedQuizData = useMemo(() => {
    if (!editingQuizDeck) return [];
    return editingQuizDeck.dataText.split(/\r?\n/).filter(line => line.trim().length > 0).map((line, idx) => {
      const match = line.match(/(=|-|:)/);
      if(!match) return { id: `q-${idx}`, line, valid: false, left: line, right: '' };
  ...
  const handleDeleteQuizItem = (indexToRemove: number) => {
    if (!editingQuizDeck) return;
        ...
        {mode === 'quiz' ? (
              ...
              <div className="flex-1 flex flex-col">
                {quizDecks.length === 0 ? (
                  <div className="bg-zinc-950 rounded-3xl p-10 text-center shadow-sm border border-zinc-900 flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full">
                    ...
                    <button 
                      onClick={() => { setMode('manage'); setEditingQuizDeck(null); }}
               ...
               {editingQuizDeck ? (
                 <div className="flex-1 flex flex-col max-w-3xl w-full mx-auto relative px-4 pb-20">
                   ...
                   <div className="space-y-3 pb-8">
                     {parsedQuizData.filter(d => d.line.toLowerCase().includes(quizEditorSearch.toLowerCase())).map((item, index) => (
          ...
          <div className="w-full flex-1 flex flex-col">
            {editingFlashcardDeck ? (
              <div className="flex-1 flex flex-col max-w-3xl w-full mx-auto relative px-4 pb-20">
                <button 
                  onClick={() => setEditingFlashcardDeck(null)}
                  className="mb-6 px-4 py-2 text-zinc-400 hover:text-zinc-200 font-medium flex items-center gap-2 transition self-start"
                >
                ...
                <div className="space-y-3">
                  {editingFlashcardDeck.words.filter(w => w.word.toLowerCase().includes(deckEditorSearch.toLowerCase()) || w.meaning.toLowerCase().includes(deckEditorSearch.toLowerCase())).map(word => (
                    <div key={word.id} className="flex justify-between items-center bg-zinc-900 border border-zinc-800 p-4 rounded-xl">
                      <div>
                        <p className="font-bold text-zinc-200">{word.word}</p>
                        <p className="text-sm text-zinc-400 mt-1">{word.meaning}</p>
                      </div>
                      <button 
                        onClick={() => handleDeleteDeckWord(word.id)}
