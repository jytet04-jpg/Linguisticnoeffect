...
export default function App() {
  ...
  const startFlashcardSession = (deck: FlashcardDeck, forceReset = false) => {
    ...
    // Initialize we progress stats for each word in this session
    const initialCorrectCount: Record<string, number> = {};
    wordsToLearn.forEach(w => {
      initialCorrectCount[w.id] = 0;
    });

    setWordCorrectCount(initialCorrectCount);
              ...
              <div className="flex-1 flex flex-col items-center justify-start max-w-2xl w-full mx-auto relative px-4 text-center pb-8 pt-0 animate-fade-in">
                 ...
                 {activeDeckTab === 'words' ? (
                   ...
                   sessionWords.length === 0 && viewingPastIndex === 0 ? (
                     <div className="flex flex-col items-center justify-center p-12 bg-zinc-900/50 rounded-3xl border border-zinc-800 w-full mt-4 text-center">
                       <CheckCircle2 size={64} className="text-emerald-400 mb-6 animate-bounce" />
                       <h3 className="text-2xl font-black text-zinc-100 mb-2 font-sans tracking-tight">Tebrikler! Seti Bitirdiniz!</h3>
                       <p className="text-zinc-400 text-sm max-w-sm mb-6">
                         Sözlükteki tüm kelimeleri başarıyla tamamladınız ve öğrendiniz!
                       </p>
                       <button 
                         onClick={() => startFlashcardSession(activeDeck, true)}
                         className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition cursor-pointer"
                       >
                         Tekrar Başla
                       </button>
                     </div>
