              ...
              <div className="flex flex-col gap-5 flex-1">
                ...
                <div className="min-h-[100px] border-2 border-dashed border-zinc-800 rounded-2xl bg-zinc-900/50 p-6 flex flex-wrap gap-3 items-center justify-center">
                  ...
                  {selectedWords.map((word, i) => (
                    <button
                      ...
                      disabled={!isSaveWordMode && !isCopyWordMode}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2.5 px-2">
                  ...
                  <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                    ...
                    >
                      <Copy size={16} />
                      {isCopyWordMode ? 'Kopyalanacak Kelimeyi Seçin...' : 'Kelime Kopyala'}
                    </button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-3 justify-center">
                  {shuffledOptions.length > 0 && shuffledOptions.map((opt) => (
                    <button
                      ...
                      disabled={(selectedWords.length === currentItem.englishWords.length) && !isSaveWordMode && !isCopyWordMode}
                      className={`px-6 py-3 bg-zinc-900 border-2 border-zinc-800 text-zinc-200 text-lg font-medium rounded-xl shadow-sm transition-colors ${
                        ...
                        errorWord === opt.word && !isSaveWordMode && !isCopyWordMode ? 'border-red-500 bg-red-950/50 text-red-400' : ''
                      }`}
                    >
                      {opt.word}
                    </button>
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-3xl w-full max-w-md flex flex-col shadow-2xl relative overflow-hidden">
            ...
            <div className="p-6 flex flex-col gap-4">
              <p className="text-sm text-zinc-400">
                Seçeceğiniz söz bir cümle mi yoksa kelime mi? Lütfen kopyalanacak formatı seçin:
              </p>
              ...
              <div className="grid grid-cols-1 gap-2.5 mt-2">
                <button
                  ...
                  className="w-full flex items-start gap-4 p-4 bg-zinc-900 hover:bg-zinc-850 border border-zinc-805 hover:border-blue-500/40 rounded-2xl transition-all group text-left cursor-pointer"
                >
                  <div className="p-2 bg-blue-500/10 text-blue-400 rounded-xl mt-0.5 group-hover:bg-blue-500/20 transition-colors">
                    <BookmarkPlus size={18} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <span className="font-semibold text-white group-hover:text-blue-400 block transition-colors text-sm">Kelime Olarak Kopyala</span>
                    <span className="text-xs text-zinc-400 mt-0.5 block">Kelimeler kopyalanırken virgülle ayrılır (örn: elma, armut, muz).</span>
                  </div>
                </button>
                ...
                <button
                  ...
                  className="w-full flex items-start gap-4 p-4 bg-zinc-900 hover:bg-zinc-850 border border-zinc-805 hover:border-blue-500/40 rounded-2xl transition-all group text-left cursor-pointer"
                >
                  <div className="p-2 bg-indigo-500/10 text-indigo-400 rounded-xl mt-0.5 group-hover:bg-indigo-500/20 transition-colors">
                    <Type size={18} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <span className="font-semibold text-white group-hover:text-blue-400 block transition-colors text-sm">Cümle Olarak Kopyala</span>
                    <span className="text-xs text-zinc-400 mt-0.5 block">Kelimeler kopyalanırken aralarına boşluk konulur, cümlenin kendi içindeki noktalama işaretleri korunur (örn: This is a sentence.).</span>
                  </div>
                </button>
              </div>
            </div>
