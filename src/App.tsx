...
export default function App() {
  ...
  const handleWordSelect = (word: string, id: string) => {
    ...
    if (isCopyWordMode) {
      if (copySelectedWords.includes(word)) {
        setCopySelectedWords(copySelectedWords.filter(w => w !== word));
      } else {
        setCopySelectedWords([...copySelectedWords, word]);
      }
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-200 font-sans selection:bg-indigo-900 flex flex-col">
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 bg-zinc-900 p-1 rounded-xl">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${mode === 'notes' ? 'bg-zinc-800 shadow-sm text-emerald-400' : 'text-zinc-400 hover:text-zinc-200'}`}
            >
              <FileText size={16} />
                ...
                <div className="min-h-[100px] border-2 border-dashed border-zinc-800 rounded-2xl bg-zinc-900/50 p-6 flex flex-wrap gap-3 items-center justify-center">
                  ...
                  {selectedWords.map((word, i) => (
                    <button
                      ...
                      disabled={!isSaveWordMode && !isCopyWordMode && !isNoteMode}
                      className={`px-5 py-3 text-lg font-medium rounded-xl shadow-sm transition-all duration-200 ${
                        isSaveWordMode 
                          ...
                          : isCopyWordMode
                          ? copySelectedWords.includes(word)
                            ? 'bg-blue-600 text-white border-2 border-blue-400 font-bold scale-[1.02] shadow-blue-500/20 shadow-md ring-2 ring-blue-500 cursor-pointer'
                            : 'bg-blue-500/10 text-blue-400 border-2 border-dashed border-blue-500 hover:bg-blue-500/20 cursor-pointer'
                          : isNoteMode
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-semibold flex items-center gap-1.5 transition-colors shadow-sm cursor-pointer ${
                        isCopyWordMode 
                          ...
                          : 'bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-blue-400 hover:bg-zinc-800'
                      }`}
                    >
                      <Copy size={16} />
                      {isCopyWordMode 
                        ? 'Kopyalanacak Kelimeyi Seçin...' 
                        : isNoteMode 
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  {shuffledOptions.length > 0 && shuffledOptions.map((opt) => (
                    <button
                      ...
                      disabled={(selectedWords.length === currentItem.englishWords.length) && !isSaveWordMode && !isCopyWordMode && !isNoteMode}
                      className={`px-6 py-3 bg-zinc-900 border-2 border-zinc-800 text-zinc-200 text-lg font-medium rounded-xl shadow-sm transition-colors ${
                        isSaveWordMode 
                          ...
                          : isCopyWordMode
                          ? copySelectedWords.includes(opt.word)
                            ? 'border-blue-500 bg-blue-950/40 text-blue-300 ring-2 ring-blue-500 scale-[1.02] shadow-lg shadow-blue-500/15 cursor-pointer border-solid font-bold'
                            : 'hover:border-blue-500 hover:bg-blue-900/10 ring-2 ring-transparent hover:ring-blue-500 cursor-pointer border-dashed'
                          : isNoteMode
                        ...
                        errorWord === opt.word && !isSaveWordMode && !isCopyWordMode && !isNoteMode ? 'border-red-500 bg-red-950/50 text-red-400' : ''
                      }`}
                    >
                      {opt.word}
                    </button>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[80] backdrop-blur-sm">
          <div className="bg-zinc-950 border border-zinc-800 rounded-3xl w-full max-w-md flex flex-col shadow-2xl relative overflow-hidden">
            ...
            <div className="p-6 flex flex-col gap-4 max-h-[75vh] overflow-y-auto">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-blue-400 block mb-2">Pano Kopyalama Seçenekleri</span>
                <div className="grid grid-cols-1 gap-2">
                  <button
                    onClick={() => {
                      setCopyFormat('word');
                      setIsCopyWordMode(true);
                      setCopySelectedWords([]);
                      setIsNoteMode(false);
                      setNoteSelectedWords([]);
                      setShowCopyFormatPicker(false);
                    }}
                    className="w-full flex items-start gap-3 p-3 bg-zinc-900 hover:bg-zinc-850 border border-zinc-850 hover:border-blue-500/40 rounded-xl transition-all group text-left cursor-pointer"
                  >
                    <div className="p-1.5 bg-blue-500/10 text-blue-400 rounded-lg mt-0.5 group-hover:bg-blue-500/20 transition-colors">
                      <BookmarkPlus size={16} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <span className="font-semibold text-white group-hover:text-blue-400 block transition-colors text-xs">Kelime Olarak Kopyala</span>
                      <span className="text-[10px] text-zinc-400 mt-0.5 block">Seçeceğiniz kelimeler kopyalanırken virgülle ayrılır (örn: elma, armut, muz).</span>
                    </div>
                  </button>

                  <button
                    onClick={() => {
                      setCopyFormat('sentence');
                      setIsCopyWordMode(true);
                      setCopySelectedWords([]);
                      setIsNoteMode(false);
                      setNoteSelectedWords([]);
                    ...
                    className="w-full flex items-start gap-3 p-3 bg-zinc-900 hover:bg-zinc-850 border border-zinc-850 hover:border-blue-500/40 rounded-xl transition-all group text-left cursor-pointer"
                  >
                    <div className="p-1.5 bg-indigo-500/10 text-indigo-400 rounded-lg mt-0.5 group-hover:bg-indigo-500/20 transition-colors">
                      <Type size={16} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <span className="font-semibold text-white group-hover:text-blue-400 block transition-colors text-xs">Cümle Olarak Kopyala</span>
                      <span className="text-[10px] text-zinc-400 mt-0.5 block">Kelimeler boşlukla ayrılır, orijinal noktalama işaretleri korunur.</span>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-400 block mb-2">Uygulama İçi Not Al (Kelimeleri Notlar Tabına Kaydet)</span>
                <div className="grid grid-cols-1 gap-2">
                  <button
                    onClick={() => {
                      setNoteFormat('word');
      ...
      {/* Multi-Word Copy Helper Bar */}
      {isCopyWordMode && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-zinc-950/95 border border-blue-500/30 rounded-2xl shadow-2xl z-40 backdrop-blur-md p-3.5 flex flex-col gap-2.5 animate-[fadeIn_0.2s_ease-out]">
          <div className="flex items-center justify-between border-b border-zinc-850 pb-2">
            <span className="text-white text-xs font-bold flex items-center gap-1.5">
              <span className="w-2 h-2 bg-blue-500 rounded-full animate-ping"></span>
              Çoklu {copyFormat === 'word' ? 'Kelime' : 'Cümle'} Seçimi ({copySelectedWords.length} Seçildi)
            </span>
            <button 
              onClick={() => {
                setIsCopyWordMode(false);
          ...
          <div className="flex gap-2 items-center">
            <span className="text-[9px] font-bold uppercase tracking-wider text-blue-400 shrink-0">METİN:</span>
            <div className="flex-1 px-2.5 py-1 bg-zinc-900 border border-zinc-850 rounded-xl max-h-[44px] overflow-y-auto text-zinc-300 text-xs font-mono truncate">
              {copySelectedWords.length > 0 ? (
                copyFormat === 'word' ? copySelectedWords.join(', ') : copySelectedWords.join(' ')
              ) : (
          ...
          <div className="grid grid-cols-2 gap-2 mt-0.5">
            ...
            <button
              onClick={() => {
                if (copySelectedWords.length === 0) {
                  setToastMessage("Kopyalamak için en az bir kelime seçin.");
                  setTimeout(() => setToastMessage(null), 2500);
                  return;
                }
                const textToCopy = copyFormat === 'word' ? copySelectedWords.join(', ') : copySelectedWords.join(' ');
                navigator.clipboard.writeText(textToCopy)
                  .then(() => {
                    setToastMessage(copyFormat === 'word' ? "Kelimeler virgülle ayrılmış olarak kopyalandı!" : "Cümle kopyalandı!");
                    setTimeout(() => setToastMessage(null), 2500);
                  })
                  .catch(err => {
              ...
              className="w-full py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold transition cursor-pointer shadow-md shadow-blue-900/40"
            >
              Kaydet (Kopyala)
            </button>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 sm:p-8 w-full max-w-sm shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <div className="w-full bg-zinc-950 border border-zinc-850 text-zinc-300 font-mono text-xs p-3 rounded-xl max-h-24 overflow-y-auto break-all">
                  {tempNoteText}
                </div>
              </div>
            </div>
